...
FastAPI application for public market discovery, SEC filings, Treasury rates,
and crypto spot references. It exposes Living Anatomy, formula, Second-Brain,
Hatun review, source-identity, and receipt contracts. Trading, wallets, custody,
and unattended effectors are intentionally absent.
"""
...
app = FastAPI(
    ...
    description=(
        ...
        "Anatomy, session-scoped memory, and non-authorizing Hatun review."
    ),
)
CLIENT = PuriqClient()
...
ANATOMY = (
    ...
    ("context", "Bind each observation to source, time, query, and session scope."),
    ("formula", "Apply explicit PURIQ formulas with maturity and proof boundaries."),
    ("policy", "Deny trading, custody, wallet, and unattended execution paths."),
    ("decide", "Produce review or abstention; never an autonomous order."),
    ("verify", "Check source identity, receipt digests, freshness, and invariants."),
    ("remember", "Retain only bounded receipt handles under a hashed session."),
...
def _require_scope(x_szl_session: str = Header(..., alias="X-SZL-Session")) -> str:
    try:
        return session_scope(x_szl_session)
...
def markets(
    ...
    try:
        ...
        raise HTTPException(502, f"Polymarket observation unavailable: {type(exc).__name__}") from exc
...
def hatun_evaluate(
    request: HatunRequest,
    x_szl_session: str = Header(..., alias="X-SZL-Session"),
) -> dict[str, Any]:
    scope = _require_scope(x_szl_session)
    memory = LEDGER.recent(scope, limit=64)
    known_receipts = {item["receipt_id"] for item in memory}
    evidence_known = bool(request.evidence_receipt_ids) and set(
        request.evidence_receipt_ids
    ).issubset(known_receipts)
    weights = [1.0 / len(request.axes)] * len(request.axes)
    score = lambda_weighted(list(request.axes.values()), weights)
    blockers: list[str] = []
    if not memory:
        blockers.append("NO_SESSION_OBSERVATIONS")
    if not request.evidence_receipt_ids:
...
def landing_page() -> str:
    ...
    formula_rows = "".join(
        f'<article class="formula"><span>{html.escape(item["id"])}</span>'
        f'<strong>{html.escape(item["name"])}</strong>'
        f'<small>{html.escape(item["status"])}</small></article>'
...
<style>
:root{{color-scheme:dark;--bg:#07050c;--deep:#030207;--panel:#151020;--panel2:#1d1629;--ink:#f7f3ff;--muted:#aa9fb7;--violet:#b989ff;--gold:#f5c86b;--cyan:#75d8ff;--line:color-mix(in srgb,var(--violet) 25%,transparent);--good:#6ee7b7;--bad:#ff7d95}}
...
.skip{{position:fixed;left:12px;top:-80px;z-index:10;background:#fff;color:#000;padding:10px 14px}}.skip:focus{{top:12px}}
...
.sources{{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:13px}}.source strong,.formula strong,.receipt strong{{display:block;font-size:17px;margin:10px 0}}.source small,.formula small,.receipt small{{color:var(--muted)}}.formula-grid{{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:12px}}.formula{{min-height:168px;display:flex;flex-direction:column}}.formula small{{margin-top:auto}}
.boundary{{margin-top:30px;padding:20px;border:1px solid var(--line);border-radius:17px;color:var(--muted);background:rgb(21 16 32/.72)}}.error{{color:var(--bad)}}footer{{display:flex;justify-content:space-between;gap:18px;flex-wrap:wrap;margin-top:44px;padding-top:22px;border-top:1px solid var(--line);color:var(--muted)}}
@media(max-width:980px){{.hero{{grid-template-columns:1fr}}.chamber{{max-width:560px}}.controls{{grid-template-columns:repeat(2,minmax(0,1fr))}}.metrics,.sources{{grid-template-columns:repeat(2,minmax(0,1fr))}}.formula-grid{{grid-template-columns:repeat(3,minmax(0,1fr))}}}}
@media(max-width:650px){{.shell{{padding-inline:18px}}h1{{font-size:clamp(54px,18vw,82px)}}.nav{{width:100%}}.nav a{{flex:1;justify-content:center}}.controls,.metrics,.grid,.sources,.formula-grid{{grid-template-columns:1fr}}.control.action button{{width:100%}}}}
@media(pointer:coarse){{a,button,input,select{{min-height:48px}}}}@media(prefers-reduced-motion:reduce){{*,*::before,*::after{{animation:none!important;transition:none!important;scroll-behavior:auto!important}}}}@media(forced-colors:active){{.ambient{{display:none}}*{{forced-color-adjust:auto}}}}
</style></head>
...
<section class="hero"><div><div class="eyebrow">PUBLIC-MARKET INTELLIGENCE / READ-ONLY</div><h1>Market Chamber.</h1><p class="lede">Prediction markets, SEC filings, Treasury rates, crypto spot references, Living Anatomy, Second-Brain receipt memory, and Hatun review—without a wallet, order path, custody surface, or fabricated feed.</p><div class="proof"><span class="pill good">SOURCE-BOUND CONTRACT</span><span class="pill">REV {html.escape(revision_short)}</span><span class="pill">LOCKED 8</span><span class="pill">Λ CONJECTURE 1</span></div></div><aside class="chamber"><div class="dial"><strong id="coverageDial">0/4</strong></div><small id="chamberState">Run the live chamber to observe public sources.</small></aside></section>
...
<section class="section"><div class="section-head"><div><div class="eyebrow">FORMULA ORGAN</div><h2>Math with limits.</h2></div><p>Formula status and non-authority are visible. CHECKED computation never upgrades Lambda uniqueness beyond Conjecture 1.</p></div><div class="formula-grid">{formula_rows}</div></section>
<section class="section"><div class="section-head"><div><div class="eyebrow">RECEIPT TAPE</div><h2>Proof of this pass.</h2></div><p>Receipts establish source bytes, normalization identity, and observation time for this pass. They do not prove every property of the world.</p></div><div id="receipts" class="grid"><article class="receipt"><span>NO RECEIPTS</span><strong>Run a live observation.</strong></article></div></section>
...
(()=>{{
 ...
 if(!session){{const bytes=new Uint8Array(32);crypto.getRandomValues(bytes);session=Array.from(bytes,b=>b.toString(16).padStart(2,'0')).join('');sessionStorage.setItem(sessionKey,session)}}
 const safe=(value)=>value===null||value===undefined?'UNAVAILABLE':String(value);
 const money=(value)=>{{const n=Number(value);return Number.isFinite(n)?new Intl.NumberFormat('en-US',{{style:'currency',currency:'USD',maximumFractionDigits:2}}).format(n):'UNAVAILABLE'}};
 const pct=(value)=>{{const n=Number(value);return Number.isFinite(n)?(n*100).toFixed(1)+'%':'UNAVAILABLE'}};
 const node=(tag,cls,text)=>{{const el=document.createElement(tag);if(cls)el.className=cls;if(text!==undefined)el.textContent=text;return el}};
 function renderSources(brief){{const root=$('sources');root.replaceChildren();const observations=brief.observations||{{}};const receipts=brief.receipts||{{}};['polymarket','sec','treasury','coinbase'].forEach(id=>{{const card=node('article','source');card.append(node('span','',id.toUpperCase()));card.append(node('strong','',observations[id]?.state==='UNAVAILABLE'?'UNAVAILABLE':'OBSERVED'));card.append(node('small','',receipts[id]?('receipt '+receipts[id].receipt_id.slice(0,12)):'No receipt closed'));root.append(card)}})}}
 function renderMarkets(brief){{const root=$('markets');root.replaceChildren();const rows=brief.observations?.polymarket?.markets||[];if(!rows.length){{const card=node('article','market');card.append(node('span','','UNAVAILABLE'));card.append(node('h3','','No current market rows were observed.'));root.append(card);return}}rows.forEach(m=>{{const card=node('article','market');card.append(node('span','',m.active&&!m.closed?'ACTIVE PUBLIC MARKET':'MARKET RECORD'));card.append(node('h3','',safe(m.question)));card.append(node('div','prob',pct(m.yes_probability)));const bar=node('div','bar');const fill=node('i');fill.style.width=Math.max(0,Math.min(100,Number(m.yes_probability||0)*100))+'%';bar.append(fill);card.append(bar);const dl=node('dl');[['Entropy',m.binary_entropy],['24h volume',money(m.volume_24h)],['Liquidity',money(m.liquidity)],['Spread',m.spread],['Data quality',m.liquidity_quality?.score]].forEach(([k,v])=>{{dl.append(node('dt','',k));dl.append(node('dd','',safe(v))}});card.append(dl);if(m.market_url){{const a=node('a','','Open public market');a.href=m.market_url;a.target='_blank';a.rel='noopener noreferrer';card.append(a)}}root.append(card)}})}}
 function renderReceipts(brief){{const root=$('receipts');root.replaceChildren();const entries=Object.entries(brief.receipts||{{}});if(!entries.length){{const card=node('article','receipt');card.append(node('span','','NO RECEIPTS'));card.append(node('strong','','All sources unavailable.'));root.append(card);return}}entries.forEach(([id,r])=>{{const card=node('article','receipt');card.append(node('span','',id.toUpperCase()));card.append(node('strong','',r.receipt_id.slice(0,16)));card.append(node('small','',new Date(r.observed_at*1000).toISOString()+' · '+r.truth_label));root.append(card)}})}}
 async function run(){{const button=$('run');button.disabled=true;$('status').textContent='Observing fixed public sources and closing receipts…';$('status').classList.remove('error');const params=new URLSearchParams({{market_limit:$('marketLimit').value,cik:$('cik').value,crypto_base:$('cryptoBase').value,crypto_currency:$('cryptoCurrency').value}});try{{const response=await fetch('/api/puriq/v1/brief?'+params.toString(),{{headers:{{'X-SZL-Session':session,'Accept':'application/json'}},cache:'no-store'}});const body=await response.json();if(!response.ok)throw new Error(body.detail||('HTTP '+response.status));$('sourcesObserved').textContent=safe(body.source_summary?.observed)+'/'+safe(body.source_summary?.total);$('marketsReturned').textContent=safe(body.observations?.polymarket?.returned);$('spotPrice').textContent=money(body.observations?.coinbase?.amount);$('lambdaScore').textContent=safe(body.lambda_advisory?.score);$('coverageDial').textContent=safe(body.source_summary?.observed)+'/'+safe(body.source_summary?.total);$('chamberState').textContent=body.status+' · '+Object.keys(body.receipts||{{}}).length+' source receipts · trading disabled';renderSources(body);renderMarkets(body);renderReceipts(body);$('status').textContent=body.complete?'Required public sources observed. Review the receipt tape below.':'Chamber degraded: '+Object.keys(body.source_failures||{{}}).join(', ');if(!body.complete)$('status').classList.add('error')}catch(error){{$('status').textContent='Observation failed closed: '+error.message;$('status').classList.add('error')}}finally{{button.disabled=false}}}}
